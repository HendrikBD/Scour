...
function! s:_resolve(promise, ...) abort
  ...
  if s:promise.is_promise(Result)
...
let s:promise = {}
let g:Promise = s:promise

function! s:promise.new(resolver) abort
...
function! s:promise.all(promises) abort
  return s:promise.new(function('s:_all', [a:promises]))
endfunction

function! s:promise.race(promises) abort
  return s:promise.new(function('s:_race', [a:promises]))
endfunction

function! s:promise.resolve(...) abort
  let promise = s:promise.new(s:NOOP)
  call s:_resolve(promise, a:0 > 0 ? a:1 : v:null)
  return promise
endfunction

function! s:promise.reject(...) abort
  let promise = s:promise.new(s:NOOP)
  call s:_reject(promise, a:0 > 0 ? a:1 : v:null)
  return promise
endfunction

function! s:promise.is_available() abort
  return has('lambda') && has('timers')
endfunction

function! s:promise.is_promise(maybe_promise) abort
  return type(a:maybe_promise) == s:DICT_T && has_key(a:maybe_promise, '_vital_promise')
endfunction

function! s:promise.wait(promise, ...) abort
...
function! s:_promise_then(...) dict abort
  ...
  let child = s:promise.new(s:NOOP)
